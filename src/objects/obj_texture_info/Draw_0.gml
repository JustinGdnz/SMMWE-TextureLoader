...
var _preview = sprite_exists(preview) ? preview : spr_Selector_TextureBanner_big;

draw_sprite_stretched(_preview, 0, x+10, y+40, 136, 80);
draw_self();
draw_sprite_stretched(_icon, 0, x+12, y+92, 26, 26);

var _name = scribble(name).starting_format("fntAseprite", c_black).align(fa_left, fa_middle);
_name.draw(x+16, y+10);

var _author = scribble(author).starting_format("fntAseprite", c_black).align(fa_left, fa_middle);
_author.draw(x+35, y+31);

var _description = scribble(description).starting_format("fntAseprite", c_black);
